...
function Fade({ Des }) {
	...
	const dimensions = [
		...
		},
		{
			...
			description:
				"네더는 기존에 이미 개척된 차원이었지만 차원 대연결 사태의 여파로 네더의 시간축이 뒤틀려 시간이 미래로 이동되었습니다. 이로 인해 네더의 환경에 맞게 진화한 여러 생물 군계를 볼 수 있습니다. 네더는 버려진 것들의 성지로도 비유되기도 하며 종족 전쟁으로 패배한 피글린족이 도망쳐 거대한 문명을 이뤄낸 곳이기도 합니다. 네더라이트를 채굴하여 막강한 힘을 가지고 그것을 기반으로 다른 차원과 활발히 교류하기도 하였지만 피글린의 수장의 욕심이 흑마법까지 그를 이끌어냈고, 결국 그 결과로 네더는 몰락했습니다. 버려진 성벽에는 흑마법의 산물인 위더 스켈레톤이, 황무지에는 여러가지로 분화된 피글린족들이 자리잡아 살아가는 중입니다.",
		},
	];
		...
		<S.Wrapper>
			<Slider {...settings}>
				{dimensions.map((item, idx) => (
					<S.Item key={idx}>
						<S.Img
							src={`../../../public/img/background/${item.img}`}
							alt={`배경 ${idx + 1}`}
						/>
						{Des && (
							<S.Description idx={4}>
